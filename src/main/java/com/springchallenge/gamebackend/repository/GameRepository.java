...
public interface GameRepository extends PagingAndSortingRepository<Game, String> {
        ...
        @Query(value = "SELECT * FROM game g "
                        + FILTER_CONDITION
                        + " ORDER BY g.release_date DESC", countQuery = "SELECT COUNT(g.id) FROM game g"
                                        + FILTER_CONDITION, nativeQuery = true)
        List<Game> findByFiltersOrderByReleaseDate(String platform, String genre, String title,
                                                   Pageable pagination);
